...
def update_module(event: ft.ControlEvent):
    _clean_page(event)
    destination_index = _get_destination_index(event)
    ...
    event.page.add(_get_controls_from_current_drawer_index(event))
    sleep(0.7)
    _close_drawer(event)
    _update_page(event)
...
def _get_controls_from_current_drawer_index(event: ft.ControlEvent):
